...
class ProoflineSkillTests(unittest.TestCase):
    def test_installer_copies_harness_assets_and_installed_init_run_works(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((vendor / "harness" / "policies" / "state.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "context.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "verification.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "recovery.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "delegation.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "candidate-search.md").is_file())
            self.assertTrue((vendor / "harness" / "policies" / "stopping.md").is_file())
            ...
            query_experience = subprocess.run(
                [
                    sys.executable,
                    "vendor/proofline/scripts/query_experience_store.py",
                    "--root",
                    str(target),
                    "--runs-dir",
                    ...
                    "json",
                    "--output",
                    "vendor/proofline/runs/sample-run/artifacts/experience-store.json",
                ],
                cwd=target,
                text=True,
                capture_output=True,
            ...
            self.assertEqual(query_experience.returncode, 0, query_experience.stderr)
            self.assertIn("Wrote CIPH experience store query", query_experience.stdout)
            experience = json.loads((vendor / "runs" / "sample-run" / "artifacts" / "experience-store.json").read_text(encoding="utf-8"))
